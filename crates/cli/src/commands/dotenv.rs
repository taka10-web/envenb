...
use anyhow::Context;
use envenb_core::VariableKind;
use envenb_core::dotenv::{Suggestion, classify, parse};
...
pub async fn import(
    ...
    if delete {
        ...
        if yes || confirm_delete(path)? {
            report_remove(ctx, envenb_core::dotenv::remove_if_covered(path, &known)?);
        } else if !ctx.json {
        ...
        println!(
            "{}",
            tr(
                "Delete the file with `envenb clean` (or `import --delete`) — EnvEnb now holds these values.",
                "ファイルは `envenb clean` (または `import --delete`) で削除できます。値は EnvEnb が保持しています。"
            )
        );
    }
    Ok(())
...
pub fn report_gitignore(ctx: &Ctx, file: &std::path::Path) {
    ...
    match envenb_core::dotenv::ensure_gitignored(&root, &[&pattern]) {
        Ok(r) if !ctx.json => {
            if !r.added.is_empty() {
                println!(
                    "{} {} → {}",
...
/// `envenb export-env [FILE]`: materialise the environment as a real dotenv file.
...
pub async fn export_env(ctx: &Ctx, file: &str, force: bool, gitignore: bool) -> anyhow::Result<()> {
    crate::human::require_human("envenb export-env")?;
    let project = ctx.current_project().await?;
    let env = ctx.current_environment().await?;
    let path = std::path::Path::new(file);
    ...
    let mut text = format!(
        "# Generated by EnvEnb for {} / {} — contains secrets, keep out of git.\n",
...
fn report_remove(ctx: &Ctx, outcome: envenb_core::dotenv::RemoveOutcome) {
    use envenb_core::dotenv::RemoveOutcome::*;
    if ctx.json {
    ...
    match outcome {
        ...
        Kept { path, uncovered } => println!(
            "{} {path} — {} {}",
            tr("Kept", "残しました:"),
            tr("not yet stored in EnvEnb:", "EnvEnb に未登録:"),
            uncovered.join(", ")
        ),
...
/// `envenb clean [DIR]`: remove dotenv files whose variables are all stored,
...
pub async fn clean(ctx: &Ctx, dir: Option<&str>, dry_run: bool, yes: bool) -> anyhow::Result<()> {
    ...
    let files = envenb_core::dotenv::dotenv_files_in(&dir);
    if files.is_empty() {
        if !ctx.json {
            println!(
                "{} {}",
    ...
    for file in files {
        ...
        let uncovered = envenb_core::dotenv::uncovered_names(&text, &known);
        ...
        if yes || confirm_delete(&file)? {
            report_remove(ctx, envenb_core::dotenv::remove_if_covered(&file, &known)?);
        } else if !ctx.json {
